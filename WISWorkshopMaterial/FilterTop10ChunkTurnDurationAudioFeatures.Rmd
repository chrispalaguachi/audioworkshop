...
rearranged_data <- d1[order(-d1$chunk_median_turn_duration), ]
...
unique_top_values <- unique(rearranged_data$chunk_median_turn_duration)
top_10_unique_values <- head(unique_top_values, 10)

subset_data <- subset(rearranged_data, chunk_median_turn_duration %in% top_10_unique_values)
...
```{r}
write.csv(subset_data, "Top10LongestChunks.csv")
